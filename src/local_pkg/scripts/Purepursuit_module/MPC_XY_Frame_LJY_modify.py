...
################################################################
def mpc_pure_pursuit(node_x, node_y,node_yaw, ego_ind, global_path_x, global_path_y,target_index):
    # v*dt = 거리 >> 내 x + 거리 = > 인덱스 찾아서 그 때의 스티어를 구하자
    # steer_list =[]
    ...
    tmp = np.degrees(math.atan2(target_y - node_y,
                        target_x - node_x)) % 360

    alpha = node_yaw - tmp
    angle = math.atan2(2.0 * P.WB * math.sin(np.radians(alpha)), 1.0)

    steer=(np.degrees(max(min(angle, 27.0), -27.0) ))
        
    return steer

def mpc_predict_next_state(z_ref, x0, y0, yaw0, v0, v_pre, delta0, ego_ind,cx,cy,direct ):
    ...
    for i in range(P.T):
        ...
        yaw1 = yaw0 + v0 / P.WB * math.tan(delta0) * P.dt
        direct = direct
        dx = [x1 - x for x in cx[ego_ind-10:ego_ind+10]]
        dy = [x1 - y for y in cy[ego_ind-10:ego_ind+10]]
        #글로벌
        dist = np.hypot(dx, dy)
        predice_ind=ego_ind-10 + int(np.argmin(dist))
        
        steer2_list.append(mpc_pure_pursuit(future_list[0,0],future_list[1,0],future_list[2,0],ego_ind,cx,cy,predice_ind))
        if v_pre == None:
            v_pre = v0
        a = (v0 - v_pre)/P.dt
        v1 = v0 + direct * a * P.dt
    ...
    return future_list,steer2_list
...
def mpc_cost_function_LJY(z_ref, z_bar, steer_list):
    ...
    for i in range(len(z_ref)):
        cost_function_1= (z_ref[0][i] - z_bar[0][i])**2 + steer_list[i]**2
        cost_function_2 = (z_ref[1][i] - z_bar[1][i])**2 + steer_list[i]**2
        cost_function_3 = (z_ref[2][i] - z_bar[2][i])**2 + steer_list[i]**2
        cost_function_4 = (z_ref[3][i] - z_bar[3][i])**2 + steer_list[i]**2
...
def main():
    global_path_x, global_path_y = read_global_path()
    cx, cy, cyaw, ck, s = cs.calc_spline_course(
        global_path_x, global_path_y, ds=P.d_dist)
