...
//   obs_marker.color.r = float(colorarr[0]) / 100;
//   obs_marker.color.g = float(colorarr[1]) / 100;
//   obs_marker.color.b = float(colorarr[2]) / 100;
...
{
  // 전체 과정: ros의 pointcloud2 -> pcl의 pointcloud2 -> pcl의 pointcloud
  // ros의 pointcloud2 -> pcl의 pointcloud2
  pcl::PCLPointCloud2 pcl_pc; // temporary PointCloud2 intermediary
  pcl_conversions::toPCL(ros_pc, pcl_pc); // (src: ros sensor_msgs, dst: pcl pointcloiud)

  // Convert point cloud to PCL native point cloud
  // pcl의 pointcloud2 -> pcl의 pointcloud
  ...
  output_ptr = pointProcessorI->FilterCloud(input_ptr);
  ...
  std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters = pointProcessorI->Clustering(output_ptr, 0.5, 7, 15000);
  ...
  for(pcl::PointCloud<pcl::PointXYZI>::Ptr cluster : cloudClusters)
  {
    ...
    // 전체 과정: pcl의 pointcloud -> pcl의 pointcloud2 -> ros의 pointcloud2
    // pcl의 pointcloud -> pcl의 pointcloud2
    pcl::PCLPointCloud2 pcl_pc2; // temporary PointCloud2 intermediary
    pcl::toPCLPointCloud2(*cluster, pcl_pc2);

    // pcl의 pointcloud2 -> ros의 pointcloud2
    sensor_msgs::PointCloud2 cluster_output;
    pcl_conversions::fromPCL(pcl_pc2, cluster_output);
    cluster_output.header.frame_id = "velodyne";
    
    // MakeVisual(cluster_output, clusterId);
    sensor_msgs::PointCloud2 input_cloud;
    ...
    // publish cluster data
    pub2.publish(cluster_output);

    ++clusterId;
  }

  // Now covert output back from PCL native type to ROS
  pcl::PCLPointCloud2 pcl_roi; // temporary PointCloud2 intermediary
  pcl::toPCLPointCloud2(*output_ptr, pcl_roi);

  sensor_msgs::PointCloud2 roi_output;
  pcl_conversions::fromPCL(pcl_roi, roi_output);
  // std::cout << ros_output << std::endl;

  // Publish cut roi data
  pub1.publish(roi_output);
  pub3.publish(markers_array);
}

int main(int argc, char** argv)
{
  cout << "2D Obstacle Detection ON\n";
  // Initialize ROS
  ros::init (argc, argv, "ObstacleDetector2d");
  ros::NodeHandle nh;
  ...
  pub1 = nh.advertise<sensor_msgs::PointCloud2> ("roi_output", 1);
