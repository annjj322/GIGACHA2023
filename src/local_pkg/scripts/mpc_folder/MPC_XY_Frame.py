...
class P:
    ...
    R = np.diag([1, 0.0001])  # penalty for inputs
    Rd = np.diag([1, 0.0001])  # penalty for change of inputs
    ...
    target_speed = 10  # target speed [km/h]
    ...
    acceleration_max = 100  # maximum acceleration [m/s2]


class Node:
    def __init__(self, x, y, yaw, v, direct=1.0):
        self.x = x
        self.y = y
        self.yaw = yaw
        self.v = v
    ...
    def update(self, a, delta, direct=1.0):
...
class PATH:
    ...
    def nearest_index(self, node):
        ...
        for i in range(len(self.cx)):
            self.dx.append(node.x - self.cx[i])
            self.dy.append(node.y - self.cy[i])
...
def calc_ref_trajectory_in_T_step(node, ref_path, sp):
    ...
    ind = ref_path.nearest_index(node)
...
def linear_mpc_control(z_ref, z0, a_old, delta_old):
    ...
    for k in range(P.iter_max): #z_bar가 예측된 거 
        z_bar = predict_states_in_T_step(z0, a_old, delta_old, z_ref)
        a_rec, delta_rec = a_old[:], delta_old[:] #rec이 옛날 꺼
...
def predict_states_in_T_step(z0, a, delta, z_ref):
    ...
    for i in range(P.NX): #x,y,v,yaw
        z_bar[i, 0] = z0[i] #초기값 대입
