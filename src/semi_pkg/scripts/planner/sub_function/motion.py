...
class Motion():
    ...
    def path_maker(self):
        ...
        if len(self.cut_path.x) > look_distance :
            ...
            # lane_offset = [3.5, 0, -3.5, -4]
            # lane_offset = [5.5, 3.5, 1.1, 0, -1.1, -2.2, -3.3]

            #### jmgay edited #####
            self.num_of_lanes = len(self.lane_offset)
            ...
            for i in range(look_distance, add_point_size):
                if i + 1 < len(self.cut_path.x):
                    ...
                    for lane_num in range(len(lane_offset)):
                        local_result = np.array([[0], [lane_offset[lane_num]], [1]])
