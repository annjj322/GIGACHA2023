...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud)
{
	...
	yroi.setInputCloud(cloud_filtered);
    yroi.setFilterFieldName("y");
    yroi.setFilterLimits(-4, 4);
    yroi.filter(*cloud_filtered);
	...
	// 	cloud_filtered->points[index].z = 0.0;
	// 	//cout << '[' << point.x << ' ' << point.y << ' ' << point.z << '[' << endl;
	// 	//cout << '[' << cloud_filtered->points[index].x << ' ' << cloud_filtered->points[index].y << ' ' << cloud_filtered->points[index].z << ']' << endl;
	// }
